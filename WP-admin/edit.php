		...
		<div class="wrapper">
			...
			<?php
				...
				if(isset($_POST['valider_course'])){
					...
					while ($donnees = $reponse->fetch())
								{
						echo '<p><a class="gris" href="../img/Courses/ComptesRendus/'.htmlspecialchars($donnees['CompteRendu']).'"><i class="gris fa fa-file-excel-o" style="font-size:18px;margin-right: 5px;"></i>'.$donnees['CompteRendu'].'</a></p>';
					}
				}
				...
				if(isset($_POST['valider_course'])){
					...
					while ($donnees = $reponse->fetch())
								{
						echo '<div><img src="../img/Courses/Résultats/'.htmlspecialchars($donnees['Resultats']).'" class="i-courses"></div>';
					}
				}
				...
				if(isset($_POST['valider_course'])){
					...
					'SELECT url
					FROM course NATURAL JOIN photos_galerie NATURAL JOIN galerie
					WHERE libelle = "'.$course.'"');

					while ($donnees = $reponse->fetch())
								{
						echo '<div class="mySlides fade">
							<img src="../img/Courses/Galerie/'.htmlspecialchars($donnees['url']).'" class="i-courses">
							</div>';
					}
				}
